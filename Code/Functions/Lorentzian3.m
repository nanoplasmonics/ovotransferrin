function [A, fc, C, k] = Lorentzian3(data, freq_delete, fc_guessing,fs)
...
%   A, fc, C, k  : 拟合参数（C 为白噪声底，k 为 log-log 斜率）
    ...
    if nargin < 4, fs = 1e5;end
    if nargin < 3, fc_guessing = 10; end

    %% 采样参数
    dt = 1/fs;           % 采样时间间隔 (s)             % 采样率 (Hz)
    data = detrend(data);
    n = length(data);
    ...
    % 仅保留前 10000 个点并去掉直流
    start_idx = 2;
    end_idx   = min(1000000, length(freq));
    ...
    % 绘图（保持原样）
    %figure;
    loglog(freq, psd/fs/fs, '-', 'MarkerSize', 2.5, 'Color', [0.1 0.3 0.9]); hold on;
    loglog(freq, model(p_fit, freq(:))/fs/fs, 'r-', 'LineWidth', 1.6);
    %loglog(freq, psd/fs/fs, '-', 'MarkerSize', 2.5);
    grid on;
    xlabel('Frequency (Hz)');
