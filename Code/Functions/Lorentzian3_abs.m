function [A, fc, C, k] = Lorentzian3_abs(data, freq_delete, fc_guessing)
...
%   A, fc, C, k  : 拟合参数（C 为白噪声底，k 为 log-log 斜率）
    ...
    % title(sprintf('Lorentzian Fit (log10):  fc = %.2f Hz,  C = %.3g', fc, C));
    ...
    ylabel('Power Spectral Density (V^2/Hz)',  'FontSize', 30);
   % title(sprintf('Lorentzian Fit (log10):  fc = %.2f Hz,  C = %.3g', fc, C));
    legend('Original PSD', 'Lorentzian + floor fit', 'Location', 'best','FontSize', 22);
